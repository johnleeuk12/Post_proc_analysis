    ...
    for ss = 1:N_pool %ss is each stim for same neuron
        ...
        if check_counter == 1
            ...
            for r = 1:length(rec_list{ss})
                ...
                SUrate{n}{ss}.mean = mean([SUrate{n}{ss}.mean ; mean(rate{ss}.stim{p},2)]);
                SUrate{n}{ss}.error = mean([SUrate{n}{ss}.error; std(rate{ss}.stim{p},1,2)/sqrt(nreps)]);
                SUrate{n}{ss}.spont = mean([ SUrate{n}{ss}.spont; mean(rate{ss}.pre{p},2) ]);
...
%                     te_ind = length(SUrate{n}{ss}.PSTH);
                    for te = 1:size(rate{ss}.PSTH,2)
                        if ~isempty(rate{ss}.PSTH{p,te})
                            SUrate{n}{ss}.PSTH{te}= [SUrate{n}{ss}.PSTH{te}; rate{ss}.PSTH{p,te}];
                        end
                    end
                    SUrate{n}{ss}.raster.stim = [SUrate{n}{ss}.raster.stim, raster{1}.stim{p}+Pool{1}(rec_list{ss}(1)).xb.nstim];
                    SUrate{n}{ss}.raster.rep = [SUrate{n}{ss}.raster.rep, raster{1}.rep{p}+ Pool{1}(rec_list{ss}(1)).xb.stimulus_ch1(1,4)];
